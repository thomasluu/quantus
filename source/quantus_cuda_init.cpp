...
void quantus_cuda_init(Distribution dist, quantus_comm<T> *comm)
{
    ...
    if (!matrix.empty()) {
        ...
        T *d_matrix;
        size_t pitch;
        cudaMallocPitch(&d_matrix, &pitch, width * sizeof(T), height);
        cudaMemcpy2D(d_matrix, pitch, h_matrix, width * sizeof(T), width * sizeof(T), height, cudaMemcpyHostToDevice);
        free(h_matrix);
        comm->matrix = d_matrix;
        comm->pitch = pitch;
